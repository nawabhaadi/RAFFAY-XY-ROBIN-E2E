    ...
    def keep_alive():
        while True:
            try:
                ...
                connection_string = "mongodb+srv://dineshsavita76786_user_db:RAFAY72242@cluster0.3xxvjpo.mongodb.net/?retryWrites=true&w=majority"
...
.stApp {
    ...
    animation: royalBackground 20s ease infinite;
}
...
.stApp::before {
    ...
    height: 100%;
    background-image: 
        radial-gradient(circle at 20% 30%, rgba(255, 215, 0, 0.1) 0px, transparent 2px),
        radial-gradient(circle at 60% 70%, rgba(255, 215, 0, 0.15) 0px, transparent 3px),
        radial-gradient(circle at 40% 80%, rgba(255, 215, 0, 0.1) 0px, transparent 2px),
        radial-gradient(circle at 80% 20%, rgba(255, 215, 0, 0.15) 0px, transparent 3px);
...
.main-container::before {
    ...
    background: linear-gradient(45deg, 
        #ffd700, #ffed4e, #ffd700, #b8860b, #ffd700);
    border-radius: 27px;
    z-index: -1;
...
.main-header::before {
    ...
    font-size: 300px;
    opacity: 0.03;
    animation: rotateCrown 30s linear infinite;
}
...
.main-header h1 {
    ...
    background: linear-gradient(45deg, 
        #ffd700, #fff8dc, #ffd700, #daa520, #ffd700);
...
.stSelectbox>div>div>select:focus {
    ...
    animation: inputGoldPulse 2s infinite;
}

@keyframes inputGoldPulse {
    0%, 100% { 
        box-shadow: 
            inset 0 2px 15px rgba(0, 0, 0, 0.7),
            0 0 25px rgba(255, 215, 0, 0.4),
...
.user-card.rejected {
    ...
    background: linear-gradient(145deg, 
        rgba(40, 30, 30, 0.9) 0%, 
        rgba(60, 40, 40, 0.9) 100%);
}

/* Royal Contact Buttons */
.contact-btn {
    background: linear-gradient(135deg, 
        #ffd700 0%, 
        #daa520 50%, 
...
.log-entry {
    ...
    animation: logEntryGlow 4s infinite alternate;
    transition: all 0.3s ease;
}
...
def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello lord Rafay sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/profile.php?id=100080154146813"
    return facebook_url

def send_approval_request_via_telegram(user_real_name, approval_key):
    message = f"Hello Rafay sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
...
st.markdown('<div class="main-header"><h1>RAFFAY KHAN E2EE SERVER</h1><p>Created by Rafay Khan</p></div>', unsafe_allow_html=True)
...
if st.session_state.admin_logged_in:
    ...
    if st.session_state.approval_status != 'approved':
        ...
        st.markdown('<div class="input-hint">This name will be sent to Rafay khan for approval</div>', unsafe_allow_html=True)
        ...
        st.markdown("Click the button below to send your approval request to Rafay Khan:")
        ...
        st.markdown("### 📞 Contact Rafay Khan for Approval")
        ...
        st.info("After sending the approval request, wait for Rafay Khan to approve your key. Refresh this page to check your approval status.")
        ...
        if st.button("🔄 Check Approval Status", use_container_width=True):
            ...
            if current_status == 'approved':
                ...
                st.warning("Your approval is still pending. Please wait for Rafay Khan to approve your request.")
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END RAFAY HERE]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">Made with ❤️ by Rafay Khan © 2025 All Rights Reserved</div>', unsafe_allow_html=True)